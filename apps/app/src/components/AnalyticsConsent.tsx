...
import { CONSENT_KEY } from '@/lib/analytics/ga4';
export function AnalyticsConsent() {
  const [visible, setVisible] = useState(false);
  const configured = Boolean(process.env.NEXT_PUBLIC_GA4_MEASUREMENT_ID?.trim());
  useEffect(() => { if (configured && localStorage.getItem(CONSENT_KEY) === null) setVisible(true); }, [configured]);
  ...
  return <aside className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-xl rounded-xl border border-white/15 bg-slate-950 p-4 text-sm text-slate-100 shadow-2xl" aria-label="Analytics consent">
    <p>May we collect anonymous usage counts with Google Analytics? We do not send file names, room or transfer IDs, contact details, or other event details. See our <Link className="underline" href="/privacy">privacy policy</Link>.</p>
