...
export function HomeView({ onCreateRoom, onJoinRoom, onPasteLink }: HomeViewProps) {
  ...
  return (
    <div className="safe-area flex flex-col h-full animate-in fade-in duration-300">
      {/* Header */}
      <header className="shrink-0 px-4 pb-2">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            {deviceId && (
              <div className="flex items-center gap-2 glass border border-(--border-soft) rounded-full py-1 pl-2.5 pr-3">
                ...
                <div className="w-7 h-7 rounded-full bg-(--color-accent-subtle) border border-(--color-accent-border) flex items-center justify-center">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-2 overflow-y-auto">
        <div className="w-full max-w-md space-y-3 sm:space-y-4">
          ...
          {pendingShareFiles.length > 0 && (
            <Card variant="bordered" className="border-(--color-accent-border) bg-(--color-accent-subtle) p-3">
      ...
      {/* Footer */}
      <footer className="shrink-0 pt-4 px-4 flex flex-col items-center gap-3 text-(--text-muted)">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-center text-xs">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 hover:text-(--color-accent-hover) transition-colors"
          >
